...
# construct the model
ppnet = torch.load(model_dir + model_file)
...
import copy
epoch = 10
log('test accuracy (with negative edges')
accu, _ = tnt.test(model=ppnet_multi, dataloader=test_loader, iter=(epoch*22)+1,
                class_specific=class_specific, log=log)

ppnet_multi.module.last_layer.weight.data = torch.clamp(ppnet_multi.module.last_layer.weight.data, min=0)

log('test accuracy (with negative edges clamped')
accu, _ = tnt.test(model=ppnet_multi, dataloader=test_loader, iter=(epoch*22)+1,
                class_specific=class_specific, log=log)
