...
from settings import num_classes, local_prototypes_per_class, pruned
...
parser.add_argument('-modeldir2', nargs=1, type=str)
parser.add_argument('-model2', nargs=1, type=str)
...
load_model_dir2 = args.modeldir2[0] #'./saved_models/vgg19/003/'
load_model_name2 = args.model2[0] #'10_18push0.7822.pth'
...
model_base_architecture2 = load_model_dir2.split('/')[2]
experiment_run2 = '/'.join(load_model_dir2.split('/')[3:])
...
load_model_path2 = os.path.join(load_model_dir2, load_model_name2)
log('load model 2 from ' + load_model_path2)
log('model 2 base architecture: ' + model_base_architecture2)
log('experiment run 2: ' + experiment_run2)

ppnet = torch.load(load_model_path)
ppnet = ppnet.cuda()
ppnet_multi = torch.nn.DataParallel(ppnet)

ppnet2 = torch.load(load_model_path2)
ppnet2 = ppnet2.cuda()
ppnet_multi2 = torch.nn.DataParallel(ppnet2)
...
if pruned == False: 
  num_prototypes_per_class = prototype_shape[0] / num_classes
  global_prototypes_per_class = num_prototypes_per_class - local_prototypes_per_class
...
for test_image_name in imgs
  ...
  logits, min_distances, _, _, _ = ppnet_multi(images_test)
  logits2, min_distances2 = ppnet_multi2(images_test)
  ...
  original_img = save_preprocessed_img(os.path.join(save_analysis_path, 'original_img.png'),
                                       images_test, idx)
##### MOST ACTIVATED (NEAREST) 10 PROTOTYPES OF THIS IMAGE, SCALED BY LAST LAYER CONNECTION TO PREDICTED CLASS
makedir(os.path.join(save_analysis_path, 'most_activated_prototypes_ll'))
log('Most activated 10 prototypes of this image (scaled):')
array_act, _ = torch.sort(prototype_activations[idx])
array_act_ll, sorted_indices_act = torch.sort(prototype_activations[idx] * ppnet.last_layer.weight[predicted_cls])
for i in range(1,11):
    log('top {0} activated prototype for this image:'.format(i))
    save_prototype(os.path.join(save_analysis_path, 'most_activated_prototypes_ll',
                                'top-%d_activated_prototype_ll.png' % i),
                   start_epoch_number, sorted_indices_act[-i].item())
    save_prototype_original_img_with_bbox(fname=os.path.join(save_analysis_path, 'most_activated_prototypes_ll',
                                                             'top-%d_activated_prototype_in_original_pimg_ll.png' % i),
                                          ...
                                          color=(0, 255, 255))
    save_prototype_self_activation(os.path.join(save_analysis_path, 'most_activated_prototypes_ll',
                                                'top-%d_activated_prototype_self_act_ll.png' % i),
                                   start_epoch_number, sorted_indices_act[-i].item())
    ...
    prototype_index = sorted_indices_act[-i].item()
    if prototype_index % num_prototypes_per_class < local_prototypes_per_class: 
        log('local prototype')
        prototype_class_identity_negc = ppnet.prototype_class_identity_negc
        _, neg_identity = torch.min(prototype_class_identity_negc[prototype_index], dim=0)
        log('negative class connection: {0}'.format(neg_identity))
    else: 
        log('global prototype')
    log('activation value (similarity score): {0}'.format(array_act[-i]))
    log('activation value (scaled by last layer weight): {0}'.format(array_act_ll[-i]))
    ...
    #plt.axis('off')
    plt.imsave(os.path.join(save_analysis_path, 'most_activated_prototypes_ll',
                            'most_highly_activated_patch_by_top-%d_prototype_ll.png' % i),
               high_act_patch)
    log('most highly activated patch by this prototype shown in the original image:')
    imsave_with_bbox(fname=os.path.join(save_analysis_path, 'most_activated_prototypes_ll',
                            'most_highly_activated_patch_in_original_img_by_top-%d_prototype_ll.png' % i),
