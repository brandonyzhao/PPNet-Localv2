...
def prune_prototypes(dataloader,
    ...
    class_of_prototypes_to_keep = \
    torch.argmax(
        prototype_network_parallel.module.prototype_class_identity[prototypes_to_keep],
        dim=1).numpy().reshape(-1)
    ...
    if copy_prototype_imgs:
        ...
        for idx in range(len(prototypes_to_keep)):
          ...
          shutil.copyfile(src=os.path.join(original_img_dir, 'prototype-img-original_with_self_act%d.png' % prototypes_to_keep[idx]),
                          dst=os.path.join(dst_img_dir, 'prototype-img-original_with_self_act%dclass%d.png' % (idx, class_of_prototypes_to_keep[idx])))
