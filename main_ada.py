...
parser.add_argument('-pos', nargs=1, type=str, default=['10'])
#parser.add_argument('-neg', nargs=1, type=float, default=[-1.])
parser.add_argument('-clst_l', nargs=1, type=float, default=[100.0])
parser.add_argument('-sep_l', nargs=1, type=float, default=[100.0])
parser.set_defaults(debug=False)
args = parser.parse_args()
os.environ['CUDA_VISIBLE_DEVICES'] = args.gpuid[0]
print(os.environ['CUDA_VISIBLE_DEVICES'])

weights = [0.1, 1.0, 2.5, 5.0, 10.0]
if int(args.pos[0]) == 10:
    pos_weight = 1.0
else: 
    pos_weight = weights[int(args.pos[0]) + 1]
neg_weight = -1. * pos_weight
clst_l = args.clst_l[0]
sep_l = args.sep_l[0]
#todo: make these variables change the loss coefficients
...
experiment_run = experiment_run + 'p' + str(pos_weight) + 'n' + str(neg_weight) + 'clst_l' + str(coefs['clst_l']) + 'sep_l' + str(coefs['sep_l']) + 'dist' + str(coefs['dist'])
...
shutil.copy(src=os.path.join(os.getcwd(), 'model_ada_dist.py'), dst=model_dir)
shutil.copy(src=os.path.join(os.getcwd(), 'train_and_test_ada.py'), dst=model_dir)
...
from settings import num_prototypes_to_add, deneg_global, add_sym, normalize_pos, normalize_neg
...
for epoch in range(num_train_epochs):
    ...
    if epoch < num_warm_epochs:
        ...
        if ppnet.num_prototypes >= ppnet.num_local_prototypes + ppnet.num_global_prototypes + num_prototypes_to_add:
            pci_new = ppnet_multi.module.get_new_pci(sep_cost_matrix, num_prototypes_to_add, deneg_global, add_sym, pos_weight, neg_weight)
        del sep_cost_matrix
    else:      
    ...
    if ppnet.num_prototypes >= ppnet.num_local_prototypes + ppnet.num_global_prototypes + num_prototypes_to_add:
        ...
        ppnet_multi.module.add_local_prototypes(pci_new, num_prototypes_to_add, normalize_pos = normalize_pos, normalize_neg = normalize_neg)
