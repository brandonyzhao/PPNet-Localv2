...
from settings import proto_k, by_class, twostep, twostepk
...
def _train_or_test(model, dataloader, writer=None, iter=0, optimizer=None, new_protos=False,class_specific=True, use_l1_mask=True,
	...
	total_tb_cost_local = 0
	total_avg_tb_cost_local = 0
	total_dist = 0
	total_l1 = 0
	...
	for i, (image, label) in enumerate(dataloader):
		...
		with grad_req:
			...
			#else: 
			if twostep and model.module.num_local_prototypes > 0.: 
				_, global_output, min_distances, dist_vec = model(input, k=proto_k, labels=target, dist_loss = True, by_class=by_class)
				prototype_activations = model.module.distance_2_similarity(min_distances)
				local_mask, counts = model.module.get_local_mask(global_output, twostepk)
				output = model.module.last_layer(local_mask * prototype_activations)
				total_local_counts += counts.sum().item()
			else: 
			...
			if global_output is None: 
				print('error: global_output is None') 
			else: 
				cross_entropy_global = torch.nn.functional.cross_entropy(global_output, target)
			...
			if class_specific:
				...
				# calculate cluster cost (global prototypes)
				...
				# calculate separation cost (global prototypes)
				...
				if model.module.num_local_prototypes > 0: 
					...
					assert (torch.sum((pwc_indicator>0.)).item() == torch.sum((pcc_indicator>0.)).item()), 'should find same number of correct and incorrect local prototypes'
					
					tb_cost_local = torch.mean(min_distances_local * (prototypes_of_correct_class_local - prototypes_of_wrong_class_local))
					n_examples_tb_local += torch.sum((pcc_indicator>0.)).item()
				else: 
					tb_cost_local = torch.tensor(0.)
				...
				if is_train:
					with torch.no_grad():
						if debug: 
							...
							sep_cost_matrix = separation * (1. - torch.eye(num_classes))
						else: 
			...
			n_correct_2 += topk_correct_batch(global_output, target, 2)
			n_correct_5 += topk_correct_batch(global_output, target, 5)

			if debug: 
				print('tb cost local', tb_cost_local.item())
			...
			total_tb_cost_local += tb_cost_local.item() * n_batches
	...
	if n_examples_tb_local != 0.: 
		log('\tlocal tb: \t{0}'.format(total_tb_cost_local / n_examples_tb_local))
	else: 
		log('\t no local tb examples!')
	log('\tk value for local prototypes: \t{0}'.format(twostepk))
	log('\tavg local prototypes for top k in example: \t{0}'.format(total_local_counts / n_examples))
...
def last_only(model, log=print):
	...
	for p in model.module.last_layer_global.parameters():
		p.requires_grad = True
