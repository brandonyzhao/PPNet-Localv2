...
import push
import prune
import train_and_test_tb as tnt
import train_and_test_diag as tntd
import save
...
parser.add_argument('-tb', nargs=1, type=float, default=[42.0])
...
experiment_run = experiment_run + 'tb' + str(coefs['tb_l']) + 'dist' + str(coefs['dist'])
...
shutil.copy(src=os.path.join(os.getcwd(), 'train_and_test_tb.py'), dst=model_dir)
...
for epoch in range(num_train_epochs):
    ...
    if ppnet.num_local_prototypes > 0.: 
        tntd.test_protos(model=ppnet_multi, dataloader=train_eval_loader, log=log, debug=args.debug)
    ...
    if ppnet.num_local_prototypes > 0.: 
        tntd.test_protos(model=ppnet_multi, dataloader=test_loader, log=log, debug=args.debug)
