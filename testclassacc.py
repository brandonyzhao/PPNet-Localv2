...
log, logclose = create_logger(log_filename = os.path.join('/usr/xtmp/brandon_zhao/PPRACNN', 'train.log'))
...
pp1_filename = './saved_models/vgg19_bn/448_local_clamp10neg10pos1.0negw-0.5noreg/10_17push0.8026.pth'
...
for i in range(140, 147):
	print('Class', i)
	sorted, sortind = torch.sort(confusion1[i], descending = True)
	for j in range(10): 
		print('Classified as class ' + str(sortind[j]) + ' frequency: ' + str(sorted[j]))

freqs = torch.sum(confusion1, 0)
sorted, sortind = torch.sort(freqs, descending = True)
print('Predicted Frequencies')
for j in range(10): 
	print('class ' + str(sortind[j]) + ' predicted frequency: ' + str(sorted[j]))
